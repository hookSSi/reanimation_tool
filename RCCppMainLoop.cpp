...
RUNTIME_COMPILER_SOURCEDEPENDENCY_FILE("reanimation-ui/reanimation_ui", ".cpp");
...
enum InterfaceIDEnumConsoleExample {
    IID_IRCCPP_MAIN_LOOP =
        IID_ENDInterfaceID,    // IID_ENDInterfaceID from IObject.h InterfaceIDEnum
...
struct RCCppMainLoop : RCCppMainLoopI,
    ...
    RCCppMainLoop() {
        ...
        g_pSys->pRuntimeObjectSystem->GetObjectFactorySystem()
            ->SetObjectConstructorHistorySize(10);
    }
    ...
    void MainLoop() override {
        ...
        ImGui::Begin("RCC++ Developer Toolsss");
        {
            ...
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip(
                    "Compilation is triggered by saving a runtime compiled "
                    "file.");
            ...
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip(
                    "RCCPPOPTIMIZATIONLEVEL_DEBUG in DEBUG, "
                    "RCCPPOPTIMIZATIONLEVEL_PERF in RELEASE.\nThis is the "
                    "default state.");
            ImGui::RadioButton("Debug", &optLevel, 1);
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip(
                    "RCCPPOPTIMIZATIONLEVEL_DEBUG\nDefault in DEBUG.\nLow "
                    "optimization, improve debug experiece.");
            ImGui::RadioButton("Performance", &optLevel, 2);
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip(
                    "RCCPPOPTIMIZATIONLEVEL_PERF\nDefaul in "
                    "RELEASE.\nOptimization for performance, debug experience "
                    "may suffer.");
            ImGui::RadioButton("Not Set", &optLevel, 3);
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip(
                    "No optimization set in compile, soeither underlying "
                    "compiler default or set through "
                    "SetAdditionalCompileOptions.");
            ...
            if (g_pSys->power_save) {
                ImGui::TextWrapped(
                    "Animated features may stop moving when Power Save is ON "
                    "and no input is detected; for example Dear ImGui Demo, "
                    "Plots Widgets");
            } else {
                ImGui::TextWrapped(
                    "Save energy by turning Power Save ON: render only when "
                    "input is detected");
            }
        }
        ImGui::End();
